...
subtitle: "Implementing the Dependency Workflow"
...
- **Objective**: Making sure auxiliary data files are up to date both in Github and our network Drive

- **Key Outputs**: Branches updated in Github, all aux data files saved in the Y drive. Notwe that the current folder storing files of hte new pipeline is saved in the option `getOption("pipaux.working_dir")`, i.e., `"Y:/PIP_ingestion_pipeline_v2"`

- **Key steps**: 
  1. Set Up the Working release 
  2. Run the Update function
  3. Explore the log to see more details of what went wrong, what worked, what checks have been carried out 

### Explanation

In this vignette, we explore the process of keeping any of the auxiliary data measures up to date with the latest changes. The process is designed in a way that it automatically manages all interdependencies among auxiliary data. This means that, when you updating a specific measure—such as `ppp`—all of its dependencies—such as `country_list`—are automatically updated as well, if needed.
...
#### More technical details on the update process 
...
## Step 3: Cehcking the log of results 
