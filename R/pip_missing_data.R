...
pip_missing_data <- function(
  ...
  if (action == "update") {
    ...
    gdp <- load_aux(maindir = maindir, branch = branch, measure = "gdp")

    pce <- load_aux(maindir = maindir, branch = branch, measure = "pce")

    pfw <- load_aux(maindir = maindir, branch = branch, measure = "pfw")

    pop <- load_aux(maindir = maindir, branch = branch, measure = "pop")

    cl <- load_aux(maindir = maindir, branch = branch, measure = "country_list")
    ...
    pop_md <-
      pop[ct_miss_data, on = c("country_code", "year")]
    ...
    dt <- pipfun::load_from_gh(
      ...
      fselect(year = year_data, country_code = code, incgroup_code)
    ...
    md[, incgroup_code := na_focb(incgroup_code), by = country_code][,
      incgroup_code := na_locf(incgroup_code),
      by = country_code
    ][,
      welfare_type := fifelse(incgroup_code == "HIC", "income", "consumption")
    ][is.na(incgroup_code), welfare_type := "consumption"][,
      incgroup_code := NULL
    ]
    ...
    # Push data (md) to GitHub as gdp.csv
    pipfun::save_to_gh(md, measure = measure, branch = branch)
