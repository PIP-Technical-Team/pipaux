...
cpi_validate_output <- function(cpi, detail = getOption("pipaux.detail.output")){
  ...
  validate(cpi, name = "CPI output data validation") %>%
    ...
    validate_if(is.character(cpi_data_level),
                description = "c`pi_data_level` should be character") %>%
    ...
    validate_if(is.numeric(cpi2011_SM23),
                description = "`cpi2011_SM23` should be numeric") %>%
    validate_if(is.numeric(cpi2017_SM23),
                description = "`cpi2017_SM23` should be numeric") %>%
